...
% filename_nav = 'data\hksc319l.22n';           
% ref_obs_path = 'data\hksc319l.22o';

% OPPO data
% filename_obs = 'oppo_urban_1';% 
% % load(['data\',filename_obs,'_nmea.mat']);
% % gt_waypoint = [rcvPosResult(1,2:4),1;...
% %                rcvPosResult(end,2:4),0];%LLH and epoch
% gt_waypoint = [22.2994003700,114.1783314100,3,1;...
%                22.2994003701,114.1783314101,3.001,5000];
% filename_nav = 'data\hksc319e.22n';           
% ref_obs_path = 'data\hksc319e.22o';

% OPPO data
filename_obs = 'oppo_urban_2';% 
% load(['data\',filename_obs,'_nmea.mat']);
% gt_waypoint = [rcvPosResult(1,2:4),1;...
%                rcvPosResult(end,2:4),0];%LLH and epoch
gt_waypoint = [22.2994648300,114.1781427800,3,1;...
               22.29946483001,114.17814278001,3.001,5000];
filename_nav = 'data\hksc319e.22n';           
ref_obs_path = 'data\hksc319e.22o';

% OPPO data
% filename_obs = 'oppo_urban_3';% 
% % load(['data\',filename_obs,'_nmea.mat']);
% % gt_waypoint = [rcvPosResult(1,2:4),1;...
% %                rcvPosResult(end,2:4),0];%LLH and epoch
% gt_waypoint = [22.2977621600,114.1789936300,3,1;...
%                22.29776216001,114.17899363001,3.001,5000];
% filename_nav = 'data\hksc319e.22n';           
% ref_obs_path = 'data\hksc319e.22o';
...
obs_path = ['data\',filename_obs,'.22o'];
global weights 
weights = 0;
global FTABLE;
...
pr1_R = pr_jump_removal(pr1_R, 50);
...
%             temp_flag = 1;
%         elseif abs(pr_ed(prn,idt)-median(pr_ed(prn,:),'omitnan'))>80 && idt > 4 && sum(flag_pr(prn,idt-3:idt-1))==3
%             temp_flag = 2;
%         end
...
% flag_change = zeros(size(pr1_R,1),size(pr1_R,2));
% for idt = 2:size(pr1_R,2)
%     for prn = 1:size(pr1_R,1)
%         if abs(pr_ed(prn,idt)-median(pr_ed(prn,:),'omitnan'))>50
%             flag_change(prn,idt) = 1;
% %             pr1_R(prn,idt) = 0;
%         end
%     end
% end  
% 
% for prn = 87:127
%     fault_epoch{prn,1} = zeros(2,size(pr1_R,2));
%     if pr1_R(prn,1)~=0
%         temp_idt = find(flag_change(prn,:)==1);
% %         fault_epoch{prn,1} = zeros(2,ceil(size(temp_idt,2)/2));
%         fault_epoch{prn,1}(1:(size(temp_idt,2)+1)) = [1,temp_idt];
%     else
%         temp_idt = find(flag_change(prn,:)==1);
% %         fault_epoch{prn,1} = zeros(2,ceil(size(temp_idt,2)/2));
%         fault_epoch{prn,1}(1:(size(temp_idt,2))) = [temp_idt];
%     end
%     fault_epoch{prn,1}(:,fault_epoch{prn,1}(1,:)==0) = [];
% end

% for prn = 87:127
%     if ~isempty(fault_epoch{prn,1}) && fault_epoch{prn,1}(2,end)~=0
%         for ide = 1:size(fault_epoch{prn,1},2)
%             pr1_R(prn,fault_epoch{prn,1}(1,ide):(fault_epoch{prn,1}(2,ide)-1)) = 0;
%         end
%     end
% end
...
%     [ls_xyz(idt,:),pr_residual{idt,1},tag] = LS_mpos_pr_resi(data_all{idt,10}(:,2),data_all{idt,10}(:,[1,3:5]),user_pos_xyz0);
    [ls_xyz(idt,:),pr_residual{idt,1},~,~] = Least_square_positioning(data_all{idt,10},user_pos_xyz0);
...
PrResi = nan(127,size(data_all,1));
PrED = nan(127,size(data_all,1));
for idt = 1:size(data_all,1)
    for idm = 1:size(data_all{idt,3},1)
        PrResi(data_all{idt,3}(idm),idt) = pr_residual{idt,1}(idm);
        if idt > 1 && pr1_R(data_all{idt,3}(idm),idt) ~= 0 && pr1_R(data_all{idt,3}(idm),idt-1) ~= 0
            PrED(data_all{idt,3}(idm),idt) = pr1_R(data_all{idt,3}(idm),idt) - pr1_R(data_all{idt,3}(idm),idt-1);
        end
    end
    check_info{idt,1} = [data_all{idt,10},data_all{idt,4},data_all{idt,9},pr_residual{idt,1}];
end
prn_valid = find(~isnan(mean(PrResi,2,'omitnan')));
figure(100);
hold on;
for prn_id = 1:size(prn_valid,1)
    plot(PrResi(prn_valid(prn_id),:));
end
...
% for prn_id = 1:size(prn_valid,1)
%     figure(100+prn_id)
%     plot(PrED(prn_valid(prn_id),:));
%     title(['PRN - ',num2str(prn_valid(prn_id))]);
%     xlabel('Epoch (s)');
%     ylabel('Pr Difference between Nearby Epochs (m)')
% end
