...
%     building = read_kml(kml);
    ...
    for i = 1:length(building)
        % 检查 center 字段是否存在且维度正确
        if ~isfield(building(i), 'center') || isempty(building(i).center) || numel(building(i).center) < 3
            warning('Building %d has an invalid or missing center field. Using default value.', i);
            building(i).center = [22.3083, 114.1854, 16]; % 填充一个默认值
        end
    end
...
function building = read_kml2(kml)
    D2R = pi/180;
    R2D = 180/pi;
