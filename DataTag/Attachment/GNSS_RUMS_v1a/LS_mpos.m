function [pos_est,sse,tag] = LS_mpos(pr,sv_pos,pos_ini)
...
elseif tag_N && ~tag_G && ~tag_B %GPS only
    [pos_est,sse] = LS_pos_N(pr,sv_pos,pos_ini);
    tag = 1;
elseif ~tag_N && tag_G && ~tag_B %GLONASS only
    [pos_est,sse] = LS_pos_G(pr,sv_pos,pos_ini);
    tag = 2;
elseif ~tag_N && ~tag_G && tag_B %BDS only
    [pos_est,sse] = LS_pos_B(pr,sv_pos,pos_ini);
    tag = 3;
elseif tag_N && tag_G && ~tag_B %GPS/GLONASS
    [pos_est,sse] = LS_pos_NG(pr,sv_pos,pos_ini);
    tag = 4;
elseif tag_N && ~tag_G && tag_B %GPS/BDS
    [pos_est,sse] = LS_pos_NB(pr,sv_pos,pos_ini);
    tag = 5;
 elseif ~tag_N && tag_G && tag_B %BDS/GLONASS
    [pos_est,sse] = LS_pos_GB(pr,sv_pos,pos_ini);
    tag = 6;
elseif tag_N && tag_G && tag_B %GPS/GLONASS/BDS
    [pos_est,sse] = LS_pos_NGB(pr,sv_pos,pos_ini);
    tag = 7;
end

function [pos_est,sse] = LS_pos_N(pr,sv_pos,pos_ini)
...
if size(pr,1)>=4
    ...
    while ((norm(delta_pos(1:3,1))>1e-3))
        for idx_sv = 1 : size(sv_pos,1)
            ...
            prn = sv_pos(idx_sv,1);
            if prn <= 32
                y(idx_sv,1) = pr(idx_sv) - pos1_pr0(idx_sv) - dt(1,1);
    ...
    pos_est = pos_ini;
    sse = (y - H*delta_pos)' * (y - H*delta_pos);%residual
    tag = 1;
end

function [pos_est,sse] = LS_pos_B(pr,sv_pos,pos_ini)
if size(pr,1)>=4
    dt = [0,0,0];
    delta_pos = [1e9; 1e9; 1e9; 0];
    while ((norm(delta_pos(1:3,1))>1e-3))
        for idx_sv = 1 : size(sv_pos,1)
            ...
            prn = sv_pos(idx_sv,1);
            if prn > 86
                y(idx_sv,1) = pr(idx_sv) - pos1_pr0(idx_sv) - dt(1,3);
                H(idx_sv,1:4) = [(pos_ini - sv_pos(idx_sv,2:4))./pos1_pr0(idx_sv),1];
            end
        end
    delta_pos = inv(H'*H)*H'*y;
    ...
    pos_est = pos_ini;
    sse = (y - H*delta_pos)' * (y - H*delta_pos);
    tag = 3;
end

function [pos_est,sse] = LS_pos_G(pr,sv_pos,pos_ini)
...
if size(pr,1)>=4
    ...
    while ((norm(delta_pos(1:3,1))>1e-3))
        for idx_sv = 1 : size(sv_pos,1)
            ...
            prn = sv_pos(idx_sv,1);
            if prn > 32 && prn <= 56
                y(idx_sv,1) = pr(idx_sv) - pos1_pr0(idx_sv) - dt(1,2);
                H(idx_sv,1:4) = [(pos_ini - sv_pos(idx_sv,2:4))./pos1_pr0(idx_sv),1];
            end
        end
    delta_pos = inv(H'*H)*H'*y;
...
function [pos_est,sse] = LS_pos_GB(pr,sv_pos,pos_ini)

% Least square positioning (BDS only)
% Input:    pr      =   pseudorange measurement
%           sv_pos  =   satellite position in ECEF
%           pos_ini =   initial guess of position in ECEF
%
    ...
    while ((norm(delta_pos(1:3,1))>1e-3))
        for idx_sv = 1 : length (sv_pos)
            ...
            if (prn > 32) && (prn <= 56)
                ...
                H(idx_sv,1:4) = [(pos_ini - sv_pos(idx_sv,2:4))./pos1_pr0(idx_sv),1];
            elseif prn > 86
                y(idx_sv,1) = pr(idx_sv) - pos1_pr0(idx_sv) - dt(1,2) - dt(1,3);
...
function [pos_est,sse] = LS_pos_NB(pr,sv_pos,pos_ini)
    ...
    while ((norm(delta_pos(1:3,1))>1e-3))
        for idx_sv = 1 : length (sv_pos)
            ...
            if prn <= 32
                ...
                H(idx_sv,1:4) = [(pos_ini - sv_pos(idx_sv,2:4))./pos1_pr0(idx_sv),1];
            elseif prn > 86
                y(idx_sv,1) = pr(idx_sv) - pos1_pr0(idx_sv) - dt(1,1) - dt(1,3);
                H(idx_sv,1:5) = [(pos_ini - sv_pos(idx_sv,2:4))./pos1_pr0(idx_sv),1,1];
            end
        end
    delta_pos = inv(H'*H)*H'*y;
...
function [pos_est,sse] = LS_pos_NG(pr,sv_pos,pos_ini)
    ...
    while ((norm(delta_pos(1:3,1))>1e-3))
        for idx_sv = 1 : length (sv_pos)
            ...
            elseif (prn > 32) && (prn <= 56)
                y(idx_sv,1) = pr(idx_sv) - pos1_pr0(idx_sv) - dt(1,1) - dt(1,2);
                H(idx_sv,1:5) = [(pos_ini - sv_pos(idx_sv,2:4))./pos1_pr0(idx_sv),1,1];
            end
        end
    delta_pos = inv(H'*H)*H'*y;
...
function [pos_est,sse] = LS_pos_NGB(pr,sv_pos,pos_ini)
