function D_coeff = UTD_coefficient_sim(rcvr_xyz,sv_xyz,diff_xyz,corner_pos,lambda)

% ------------------------------------------------------------------------
% The Uniform Theory of Diffraction simulation on GNSS
% Output Diffraction coefficient
%
...
% phi & phi_ correction (projection)
v_t0 = cross(v_n_prime,v_e);
v_st = (v_s-dot(v_s,v_e)*v_e)/norm(v_s-dot(v_s,v_e)*v_e);
v_st_prime = (v_s_prime-dot(v_s_prime,v_e)*v_e)/norm(v_s_prime-dot(v_s_prime,v_e)*v_e);
phi_ = 180-(180-acosd(dot(-v_st_prime,v_t0)))*sign(dot(-v_st_prime,v_n_prime));
phi = 180-(180-acosd(dot(v_st,v_t0)))*sign(dot(v_st,v_n_prime));%angle overflow
...
D_coeff = [D3_RR*sqrt(1/s),D3_RR,D3_sh];
