...
%load PRN masks 
[iodp_mask, prn_mask] = load_prnmask(MT, msg); % MT == 1

%load IGP masks 
[iodi_mask, band_mask, igp_mask] = load_igpmask(MT, msg); % MT == 18

%load fast corrections (or pseudorange corrections)
[prc_E, UDREI_E, GPS_time_fc] = load_fc(iodp_mask, prn_mask, MT, msg, time_E); % MT == 2 | MT == 3 | MT == 4 | MT == 24 | MT == 0

%load long term corrections (delta position SV and delta clock SV)
[dx_E, dy_E, dz_E, doffset_E, iode_E, GPS_time_ltc] = load_ltc(iodp_mask, prn_mask, MT, msg, time_E); % MT == 25 | MT == 24

%load the coordinates of IGP nodes and the corresponding vertical ionospheric correction
[igp, ivd_E, lat_igp, lon_igp, GPS_time_ic] = load_ic(iodi_mask, band_mask, igp_mask, MT, msg, time_E); % MT == 26

% Dabin Need to revise MT == 10 | MT == 27 | MT == 28 | MT == 7
...
[E_UDREI] = sync_ER(UDREI_E, GPS_time_fc, gps_week, gps_tow);
...
sbas.prc = E_prc; %load fast corrections (or pseudorange corrections)
sbas.UDREI = E_UDREI;
sbas.dx  = E_dx; %load long term corrections (delta position SV and delta clock SV)
sbas.dy  = E_dy;
sbas.dz  = E_dz;
sbas.doffset = E_doffset;
sbas.iode = E_iode;
sbas.ivd  = E_ivd; %load the coordinates of IGP nodes and the corresponding vertical ionospheric correction
sbas.igp  = igp;
