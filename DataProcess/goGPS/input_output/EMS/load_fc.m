function [prc_E, UDREI_E, GPS_time02] = load_fc(iodp_mask, prn_mask, MT, msg, GPS_time)
...
for i = 2 : n02
    prc_E(i,:) = prc_E(i-1,:);
    UDREI_E(i,:) = UDREI_E(i-1,:);
    for j = 1 : n_SV(i)
        if PRN(i, j) > 0
            UDREI_E(i, PRN(i, j)) = UDREI(i,j);
            if UDREI(i,j) < 14
