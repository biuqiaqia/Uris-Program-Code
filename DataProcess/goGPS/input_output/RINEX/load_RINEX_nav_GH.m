function [Eph, iono] = load_RINEX_nav_GH(filename, constellations, flag_SP3, wait_dlg)
...
if (~flag_SP3)
    ...
    if (constellations.Galileo.enabled)
        if (exist([filename(1:end-1) 'l'],'file'))
            ...
            [Eph_E, iono_E] = RINEX_get_nav_GH([filename(1:end-1) 'l'], constellations);
            Eph_E(30,Eph_E(30,:)>86) = Eph_E(30,Eph_E(30,:)>86)-86+300;% new Galileo starts from 300 
        elseif (~flag_mixed)
    ...
    if (constellations.BeiDou.enabled)
        if (exist([filename(1:end-1) 'b'],'file'))
            ...
            Eph_C(30,Eph_C(30,:)>123) = Eph_C(30,Eph_C(30,:)>123)-123+400;% new Galileo starts from 400 
        elseif (~flag_mixed)
