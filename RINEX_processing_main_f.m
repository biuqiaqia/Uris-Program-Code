...
% gt_waypoint = [rcvPosResult(1,2:4),1;...
%                rcvPosResult(end,2:4),0];%LLH and epoch
gt_waypoint = [22.2972490 , 114.1781895, 5, 1; ... 
               22.297249001, 114.178189501, 5.001, 1000]; 
filename_nav = 'NewData/ephmeris/2024Da34/hksc091h.25n';           
ref_obs_path = 'NewData/ephmeris/2024Da34/2024Da34.25o';


% Processing Setup        
% file_path = 'data\building_model\demo_HK_TST_fix.kml';
% skymask_path = 'data\building_model\skymask_TST_ned';
ref_xyz = [-2418127.8635,5386283.4714,2404949.2744]; % hksc
...
GPS_flag = 1;  GLO_flag = 0;  GAL_flag = 1;  BDS_flag = 0;  QZS_flag = 0; SBS_flag = 0;
...
%     pr1(id_mpr,1)=0;
    ...
    data_all{t,2} = [rcr_loc_temp(1)*R2D rcr_loc_temp(2)*R2D rcr_loc_temp(3)]; % location that receiver estimated (LLA)
...
% 初始化变量
max_prn_list = NaN(size(data_all, 1), 1); % 存储每个时间点的最大PRN
max_residual_list = NaN(size(data_all, 1), 1); % 存储每个时间点的最大残差
row_counter = 1; % 行号计数器


for idt = 1:1:size(data_all,1)
    if ~isempty(data_all{idt,2}) 
        wls_llh(idt,1) = data_all{idt,2}(1);
        wls_llh(idt,2) = data_all{idt,2}(2);
        ...
        %
        time = data_all{idt, 1}; % GPS时间
        prns = data_all{idt, 3}; % 可见卫星的PRN列表
        % 检查伪距残差是否为空
        if ~isempty(pr_residual{idt, 1})
            % 找到偏离最大的PRN
            pr_residuals = pr_residual{idt,1};  % 提取当前历元的伪距残差
            prns = data_all{idt,3};             % 提取当前历元的PRN列表
    
            % 找到最大残差对应的PRN
            [max_residual, max_idx] = max(abs(pr_residuals));
            max_prn = prns(max_idx);
    
            % 存储结果
            max_prn_list(idt) = max_prn;           % 存储最大PRN
            max_residual_list(idt) = max_residual; % 存储最大残差
    
            % 输出每个PRN的计算结果
            %{
            for idm = 1:length(prns)
                fprintf('Row: %d, Time: %d, PRN: %d, ', row_counter, time, prns(idm)); % 行号、时间和PRN
                fprintf('Residual: %.2f, ', pr_residuals(idm)); % 伪距残差
                fprintf('Max Residual: %.2f (PRN: %d)\n', max_residual, max_prn); % 最大伪距残差及其PRN
                row_counter = row_counter + 1; % 更新行号
            end
            fprintf('----------------------------------------\n');
            %}
        end
...
output_filename = fullfile(output_folder, sprintf('%s.csv', filename_obs));
...
fprintf(fid_out, 'GPS_Time,PRN,Elevation,Azimuth,CNR,Pr_Residual,LS_Latitude,LS_Longitude,LS_Height,GT_Latitude,GT_Longitude,GT_Height,Doppler,SaVX,SaVY,SaVZ,lambda,corrected_Pr,SaX,SaY,SaZ,Max_PRN,Max_Residual\n');

for idt = 1:size(data_all, 1)
    if isempty(data_all{idt, 2}) || isempty(pr_residual{idt, 1})
        continue;
    end

    if size(data_all{idt, 3}, 1) ~= size(pr_residual{idt, 1}, 1)
        continue; 
    end

    for idm = 1:size(data_all{idt, 3}, 1)
        fprintf(fid_out, ...
            '%d,%d,%6.2f,%6.2f,%6.2f,%18.15f,%18.15f,%18.15f,%18.15f,%18.15f,%18.15f,%18.15f,%6.2f,%18.15f,%18.15f,%18.15f,%6.2f,%18.15f,%18.15f,%18.15f,%18.15f,%d,%.2f\n', ...
            ...
            pr_residual{idt, 1}(idm), ... % Pr_Residuals
            data_all{idt, 2}(1), ... % WLS_Latitude
            data_all{idt, 2}(2), ... % WLS_Longitude
            data_all{idt, 2}(3), ... % WLS_Height
            gt_lat_epoch(idt), ... % GT_Latitude
            gt_lon_epoch(idt), ... % GT_Longitude
            gt_waypoint(1, 3), ... % GT_Height
            data_all{idt, 11}(idm, 1), ... % doppler
            data_all{idt, 11}(idm, 2), ... % SaVX
            data_all{idt, 11}(idm, 3), ... % SaVY
            data_all{idt, 11}(idm, 4), ... % SaVZ
            data_all{idt, 11}(idm, 5), ... % lambda
            data_all{idt, 10}(idm, 2), ... % corrected_Pr
            data_all{idt, 10}(idm, 3), ... % SaX
            data_all{idt, 10}(idm, 4), ... % SaY
            data_all{idt, 10}(idm, 5), ... % SaZ
            max_prn_list(idt), ... % Max_PRN
            max_residual_list(idt) ... % Max_Residual
        );
    end
end
fclose(fid_out);

disp(['file kept: ', output_filename]);
